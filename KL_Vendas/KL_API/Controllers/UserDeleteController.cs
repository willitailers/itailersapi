...
namespace KL_API.Controllers
{
    public class UserDeleteController : ApiController
    {
        ...
        [HttpPost]
        public HttpResponseMessage Post([FromBody]UserDelete ativacao)
        {
            try
            {
                ...
                // Passou da validação
                var retorno = new Ativacao_Controle().deleteUser(ativacao, client);
            ...
            {
                new Ativacao_Controle().log_inserir("Erro ativacao " + ex.Message, (int)Lista_Erro.user_delete);
